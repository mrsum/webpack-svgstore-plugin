...
WebpackSvgStore.prototype.apply = function(compiler) {
  ...
  var analyzeAst = function(expr) {
    ...
    var run = function(resolve, expr) {
      ...
      spriteFolder = path.join(this.state.current.context, props.path || '');
      ...
      return function(unused, done) {
        done();
      };
    };

    new Promise(function(resolve) {
      compiler.plugin('run', run.bind(_self)(resolve, expr));
      // compiler.plugin('watch-run', run.bind(_self)(resolve, expr));
    })
    .then(function(data) {
      tasks.push(data);
    });
